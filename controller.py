...
from panda3d.core import CollisionNode, GeomNode, CollisionRay, CollisionHandlerQueue, CollisionTraverser, WindowProperties
...
class Main(ShowBase):
    ...
    def tick(self, task):
        ...
        if self.player:
            # if 'spin' in self.input_events:
            #     self.player.z_rotation += 10

            # if 'jump' in self.input_events and not getattr(self.player, 'is_jumping', False):
            #     self.player.is_jumping = True
            #     self.player.jump_progress = 0

            if 'move_forward' in self.input_events:
